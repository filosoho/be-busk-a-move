...
describe("PATCH", () => {
	test("PATCH 200, alters the busk location when given a busk_id and returns the updated busk", () => {
		const body = { busk_location_name: "West One, Leeds" };
		return request(app)
			.patch("/api/busks/2")
			.send(body)
			.expect(200)
			.then(({ body }) => {
				expect(body.busk).toEqual({
					busk_id: 2,
					busk_location: {
						latitude: -4.2768,
						longitude: 168.4704,
					},
					busk_location_name: "West One, Leeds",
					busk_time_date: "2024-08-14T23:00:00.000Z",
					username: "Lilian_Padberg35",
					user_image_url: "https://avatars.githubusercontent.com/u/50587032",
					busk_about_me:
	...
	test("PATCH 400, responds with a 400 error if there is valid body fields but with invalid values", () => {
		const body = { busk_location_name: null };
		return request(app)
			.patch("/api/busks/2")
			.send(body)
			.expect(400)
