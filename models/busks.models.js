...
exports.selectBusks = (
	...
	if (instrument) {
		return checkInstrumentExists(instrument).then((result) => {
			if (result === false) {
				return Promise.reject({ status: 400, msg: "Bad request" });
			} else {
				...
				return db.query(sqlQuery).then(({ rows }) => {
					const filteredBusks = rows.filter((busk) =>
						busk.busk_selected_instruments.includes(instrument)
					);
